...
// Only counts jobs with `verified: true` — see lib/smoke-test.js's isVerified for what
// that means (smoke-tested clean at the job's current version). Jobs missing the field
// entirely (e.g. a caller that hasn't run smoke-test tracking) count as unverified.
export function countVerifiedByCategory(jobs) {
  const counts = new Map();
  for (const j of jobs) if (j.verified) counts.set(j.category, (counts.get(j.category) ?? 0) + 1);
  return counts;
}
...
export function buildSummaryLines(jobs, categoryDescriptions) {
  ...
  const verifiedByCategory = countVerifiedByCategory(jobs);
  const verifiedTotal = jobs.filter((j) => j.verified).length;
  const categories = [...byCategory.keys()].sort();
  return [
    `${jobs.length} jobs across ${categories.length} categories (${verifiedTotal} smoke-tested clean):`,
    "",
    "| category | jobs | smoke-tested | description |",
    "|---|---|---|---|",
    ...categories.map(
      (c) =>
        `| \`${c}\` | ${byCategory.get(c)} | ${verifiedByCategory.get(c) ?? 0} | ${categoryDescriptions[c] ?? ""} |`
    ),
  ];
}
