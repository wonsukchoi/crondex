...
Usage:
  ...
  crondex deploy <id> [--target crontab|github-actions|systemd|docker|k8s-cronjob|
                                 terraform|eventbridge|cloud-scheduler]
                      [--mode script|prompt]
  ...
  crondex bundle <file.yaml> [--target crontab|github-actions|systemd|docker|k8s-cronjob|
                                        terraform|eventbridge|cloud-scheduler] [--dry-run]
...
kubectl-applied. --target eventbridge and --target
...
function deploy(id, flags) {
  ...
  if (target === "crontab") {
    ...
    console.error(
      `unknown --target "${target}" — use "crontab", "github-actions", "systemd", "docker", "k8s-cronjob", "terraform", "eventbridge", or "cloud-scheduler".`
    );
    process.exit(1);
  }
}
