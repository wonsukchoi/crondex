...
export const COMMANDS = {
  ...
  bundle: (positional, flags) => {
    if (!positional[0]) usageError("usage: crondex bundle <file.yaml> [--target <target>] [--dry-run] [--out-dir <path>] [--install]");
  ...
  mcp: (positional, flags) => {
    ...
    import("./mcp-server.js").then((m) =>
      m.startMcpServer({ allowDeploy: hasFlagVal(flags, "allow-deploy") })
    );
  },
};
