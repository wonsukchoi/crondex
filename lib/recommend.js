...
const SYNONYM_GROUPS = [
  ...
  // Merged with "renew"/"renewal" — a query about a lapsing/renewing thing (domain,
  // cert, contract) should match jobs framed either as "expiry" or "renewal" alerts.
  ["expire", "expiry", "expiration", "lapse", "renew", "renewal"],
  // "archive" added alongside backup/snapshot — same catalog concept (retention:54 tag).
  ...
  ["stale", "outdated", "old"],
  // New groups below, grounded in catalog.json tag vocabulary (checked via a tag
  // frequency scan) to close gaps where a query's word choice doesn't match how
  // job authors tagged/described the same concept.
  ["certificate", "certification"], // certification:24 tag; kept separate from "cert"/"ssl"/"tls"
  // since those already disambiguate specific job flavors in the real catalog.
  ["license", "licensing", "permit"], // license:8, licensing:22, permit:3 tags
  ["warranty", "guarantee"], // warranty:8 tag
  ["subscription", "membership"], // subscription:2, membership:15 tags
  ["vendor", "supplier"], // vendor:1, supplier(s):7 tags
  ["credential", "password", "access"], // credentials:2, access:2 tags
  ["capacity", "utilization", "occupancy"], // capacity:20, utilization:14, occupancy:4 tags
  ["headcount", "staffing", "roster"], // staffing:65, headcount:7, roster:1 tags
];
