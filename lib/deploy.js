...
// syntax almost unchanged — two gotchas: `*/n`, which systemd requires written as
// `0/n` (a bare `*` can't carry a step); and `a-b` ranges, which systemd requires
// written as `a..b` (a bare hyphen is a date-literal separator like `2012-10-10`,
// not a range operator — a plain `-` here silently produces a value systemd either
// rejects or, worse, mis-parses). `a-b/n` step-in-range carries the same fix plus
// the existing step rewrite.
function translateNumericPart(part) {
...
function translateNumericField(field) {
  return field.split(",").map(translateNumericPart).join(",");
}

// Cron's day-of-week field is numeric (0-6 or 7, both Sun); systemd wants weekday
// names, and ranges written with `..` rather than cron's `-` (see translateNumericPart
// above — same hyphen-isn't-a-range-operator gotcha). Only handles the shapes crondex
// jobs actually use (digit, list, a-b range) — not the rarer `a-b/n` step-in-range cron
// syntax, which is explicitly rejected below rather than silently mistranslated
// (Number("1-5/2") is NaN, so DOW[NaN % 7] would otherwise produce the literal string
// "undefined" in the generated OnCalendar=).
