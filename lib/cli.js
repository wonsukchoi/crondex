...
Usage:
  crondex list [--category <name>] [--tag <name>] [--verified-only] [--json]
  crondex categories [--json]
  crondex show <id> [--json]
  crondex next <id> [--count <n>] [--json]
  crondex add <id> [--dest <path>]
  crondex recommend "<what you want done>" [--limit <n>] [--verified-only] [--json]
  ...
  crondex deploy <id> [--target crontab|github-actions|systemd|docker|k8s-cronjob|
                                 terraform|nomad|eventbridge|cloud-scheduler]
                      [--mode script|prompt]
  ...
  crondex bundle <file.yaml> [--target crontab|github-actions|systemd|docker|k8s-cronjob|
                                        terraform|nomad|eventbridge|cloud-scheduler] [--dry-run]
...
Examples:
  ...
  crondex deploy repo-health-check --target terraform --dest ./terraform/repo-health-check.tf
  crondex deploy repo-health-check --target nomad --dest ./nomad/repo-health-check.nomad.hcl
...
--verified-only on list/recommend restricts results to jobs smoke-tested
clean at their current version (maintainer runs \`npm run smoke-test\`
locally and commits the result — not run in CI, since some job defaults
make real network calls). show prints a job's verified status too. A job
with no ✓ isn't necessarily broken — it may just not have been
smoke-tested yet, or have been edited since its last pass.
...
./k8s/<id>.cronjob.yaml); --target terraform writes a Terraform
kubernetes_cron_job_v1 resource (requires the Kubernetes provider,
default ./terraform/<id>.tf) — same job, IaC-managed instead of
kubectl-applied. --target nomad writes a Nomad periodic batch job spec
(default ./nomad/<id>.nomad.hcl) — run with \`nomad job run\`. --target
eventbridge and --target cloud-scheduler print a ready aws/gcloud CLI
command instead — those
...
function recommend(queryText, flags) {
  ...
  const verifiedOnly = hasFlagVal(flags, "verified-only");
  ...
  const candidates = verifiedOnly ? CATALOG.jobs.filter((j) => j.verified) : CATALOG.jobs;
  const ranked = rankJobs(candidates, queryText, limit);
  if (!ranked.length) {
    if (json) return printJson([]);
  ...
  for (const r of ranked) {
    console.log(
      `${r.job.id}  [${r.job.category}]  score ${r.score}  (${r.job.modes.join(", ")})${r.job.verified ? "  ✓ smoke-tested" : ""}`
    );
...
function list(flags) {
  ...
  const jobs = CATALOG.jobs.filter(
    (j) => (!category || j.category === category) && (!tag || j.tags.includes(tag)) && (!verifiedOnly || j.verified)
  );
  ...
  for (const j of jobs) {
    console.log(`${j.id}  [${j.category}]  (${j.modes.join(", ")})${j.verified ? "  ✓ smoke-tested" : ""}`);
...
function show(id, flags) {
  ...
  if (hasFlagVal(flags, "json")) return printJson({ ...yaml.load(raw), verified: meta.verified });
  console.log(
    meta.verified ? "✓ smoke-tested clean at this version" : "not smoke-tested (or edited since its last pass)"
  );
  console.log();
  console.log(raw);
}
...
function deploy(id, flags) {
  ...
  if (target === "crontab") {
    ...
    const hcl = buildTerraformKubernetesCronJob(doc, mode === "prompt" ? prompt : command, mode === "prompt");
    const dest = flagVal(flags, "dest") ?? join("./terraform", `${id}.tf`);
    writeArtifacts(
      [[dest, hcl]],
      `wrote ${dest} — apply with:\n  terraform apply -target=kubernetes_cron_job_v1.${id.replace(/-/g, "_")}`
    );
  } else if (target === "nomad") {
    const hcl = buildNomadPeriodicJob(doc, mode === "prompt" ? prompt : command, mode === "prompt");
    const dest = flagVal(flags, "dest") ?? join("./nomad", `${id}.nomad.hcl`);
    writeArtifacts([[dest, hcl]], `wrote ${dest} — run with:\n  nomad job run ${dest}`);
    ...
    console.error(
      `unknown --target "${target}" — use "crontab", "github-actions", "systemd", "docker", "k8s-cronjob", "terraform", "nomad", "eventbridge", or "cloud-scheduler".`
    );
    process.exit(1);
  }
}
