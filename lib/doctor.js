  ...
  for (const line of lines) {
    ...
    if (!job) {
      ...
      if (job.schedule !== schedule) {
        ...
        details.push(`installed schedule "${schedule}" differs from the catalog's "${job.schedule}" — redeploy to pick it up.`);
      }
      if (version === undefined) {
        ...
        details.push(`installed version ${version} is behind the catalog's version ${job.version} — redeploy to update.`);
      }
    }
