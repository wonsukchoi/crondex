...
export function createServer({ allowDeploy = false } = {}) {
    ...
    "crondex_recommend",
    {
      ...
      inputSchema: {
        ...
        verified_only: z
          .boolean()
          .optional()
          .describe("Only match jobs smoke-tested clean at their current version (see crondex_list)."),
      },
    },
    async ({ query, limit, verified_only }) => {
      if (!tokenize(query).length) {
        return errorResult(
          "query too vague to match on — describe what you want the job to check or remind you about."
        );
      }
      const candidates = verified_only ? CATALOG.jobs.filter((j) => j.verified) : CATALOG.jobs;
      const ranked = rankJobs(candidates, query, limit ?? 5);
      if (!ranked.length) {
  ...
  server.registerTool(
    ...
    {
      description:
        "Browse crondex jobs, optionally filtered by category and/or tag. Returns job metadata (not the full YAML — use crondex_show for that). Each job includes `verified`: whether it's been smoke-tested clean at its current version.",
      inputSchema: {
        ...
        verified_only: z.boolean().optional().describe("Only return jobs smoke-tested clean at their current version."),
      },
    },
    async ({ category, tag, verified_only }) => {
      const jobs = CATALOG.jobs.filter(
        (j) =>
          (!category || j.category === category) && (!tag || j.tags.includes(tag)) && (!verified_only || j.verified)
      );
  ...
  server.registerTool(
    ...
    async ({ id }) => {
      ...
      return textResult({ ...yaml.load(raw), verified: meta.verified });
    }
  );
      ...
      async ({ id, target, vars, mode }) => {
        ...
        switch (resolvedTarget) {
          ...
          case "k8s-cronjob":
            ...
            break;
          case "terraform":
            artifacts = buildTerraformKubernetesCronJob(doc, text, isPrompt);
            break;
          case "nomad":
            artifacts = buildNomadPeriodicJob(doc, text, isPrompt);
            break;
          case "eventbridge":
